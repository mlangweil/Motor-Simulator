...
// ==============================================================
`timescale 1ns/1ps
module calculateCoefficients_control_s_axi
#(parameter
...
//------------------------Parameter----------------------
localparam
    ADDR_LOWERCUTOFF_DATA_0  = 6'h10,
    ADDR_LOWERCUTOFF_CTRL    = 6'h14,
    ADDR_UPPERCUTOFF_DATA_0  = 6'h18,
...
always @(posedge ACLK) begin
    if (ACLK_EN) begin
        if (ar_hs) begin
            rdata <= 'b0;
            case (raddr)
                ADDR_LOWERCUTOFF_DATA_0: begin
                    rdata <= int_lowerCutoff[31:0];
                end
                ADDR_UPPERCUTOFF_DATA_0: begin
                    rdata <= int_upperCutoff[31:0];
                end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_lowerCutoff[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_LOWERCUTOFF_DATA_0)
            int_lowerCutoff[31:0] <= (WDATA[31:0] & wmask) | (int_lowerCutoff[31:0] & ~wmask);
    end
end

// int_upperCutoff[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_upperCutoff[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_UPPERCUTOFF_DATA_0)
            int_upperCutoff[31:0] <= (WDATA[31:0] & wmask) | (int_upperCutoff[31:0] & ~wmask);
    end
end

// int_samplingRate[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_samplingRate[31:0] <= 0;
    else if (ACLK_EN) begin
